...
function extractFrontMatter(text) {
  ...
  if (matches.length < 2 || matches[0].index !== 0) return { front: null, rest: text };
...
function convertHtmlComments(slideContent) {
  // convert <!-- ... --> into their inner content, with [Click] removed
  return slideContent.replace(/<!--([\s\S]*?)-->/g, (_, inner) => inner.replace(/\[Click\]/gi, '').trim());
}
...
async function run() {
  ...
  for (let s of slidev.slides) {
    ...
    // convert html comments -> text and remove [Click]
    slideContent = convertHtmlComments(slideContent);
